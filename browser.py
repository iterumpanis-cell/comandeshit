        ...
        async with self._lock:
            try:
                ...
                async def load_rows():
                    ...
                    rows = await main.evaluate(
                        """() => Array.from(document.querySelectorAll('input[name^="articulo"]')).map(input => {
                            ...
                            return {
                                suffix,
                                article_code: value('articulo'),
                                requested: value('qd'),
                                served: value('qs'),
                                returned: value('qt'),
                                order_type: value('tipuscomanda'),
                                id: value('idServit'),
                                paper_href: paper?.getAttribute('href') || ''
                            };
                        })"""
                    )
                    return rows, main
                ...
                if len(target) != 1:
                    return {
                        "ok": False,
                        "error": f"No s'ha identificat una única línia ({len(target)} coincidències).",
                        "matches": target,
                    }
    ...
    async def actualitzar_linia(self, client: str, data: str, article_code: int,
        ...
        async with self._lock:
            try:
                ...
                # La web guarda en fer blur. Assignem D i S abans de disparar
                # goNext perquè l'única petició ja contingui els dos valors.
                await main.evaluate(
                    """({suffix, quantity}) => {
